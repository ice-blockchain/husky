...
generate:
	...
	swag init --parseDependency --parseInternal -d cmd/husky-pack -g husky_pack.go -o cmd/husky-pack/api
	swag fmt -d cmd/husky-pack -g husky_pack.go
...
benchmark:
	# TODO make -race work
	go test -tags=go_json -run=^$ -v -bench=. -benchmem -benchtime 10s ./cmd/husky-pack
